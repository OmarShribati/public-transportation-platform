...
const FormRepeater = ({ name, field, formik, label }: RepeaterFieldProps) => {
  ...
  useEffect(() => {
    ...
    if (!field.startCollapsed && currentList.length === 0) {
      const count = field.defaultCount || 1;
      const newValues = { ...formik.values };
      const defaultArray = Array.from({ length: count }, () => createDefaultItem(field.options));
      set(newValues, name, defaultArray);
      formik.setValues(newValues);
    }
  }, [field.options, field.startCollapsed, formik.values, name]);
  ...
  const handleAdd = () => {
    ...
    if (!isExpanded) {
      const count = field.defaultCount || 1;
      set(newValues, name, Array.from({ length: count }, () => createDefaultItem(field.options)));
      formik.setValues(newValues);
      setIsExpanded(true);
      return;
    }

    set(newValues, name, [...currentList, createDefaultItem(field.options)]);
    formik.setValues(newValues);
  };
