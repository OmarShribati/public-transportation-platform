  ...
  return (
    <div className="flex items-center gap-1.5 px-2">
      {type?.map((action) => {
        ...
        const getBtnBg = (act: string) => {
          if (act.includes('delete')) return 'hover:bg-red-500/20 text-red-500';
          if (act.includes('edit')) return 'hover:bg-blue-500/20 text-blue-500';
          return 'hover:bg-emerald-500/20 text-emerald-500';
        };

        return (
          <div key={action} className="relative">
            {React.cloneElement(button as React.ReactElement, {
              className: `w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 ${getBtnBg(action)}`
            })}
          </div>
        );
      })}
