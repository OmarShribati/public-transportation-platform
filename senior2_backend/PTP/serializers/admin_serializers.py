...
from PTP.models import Driver, Route, RouteStop, Stop, User, Vehicle, Zone
...
class AdminAccountCreateSerializer(serializers.Serializer):
    ...
    account_type = serializers.ChoiceField(
        choices=['passenger', 'driver', 'merchant']
    )
    ...
    def validate(self, attrs):
        ...
        if (
            ...
            or Driver.objects.filter(email=email).exists()
        ):
        ...
        if account_type == 'passenger':
            ...
            passenger_only_excluded_fields = [
                'address',
                'id_card_image_1',
                'id_card_image_2',
                'license_image',
            ...
            for field in passenger_only_excluded_fields:
        ...
        if account_type == 'merchant':
            ...
            if not address:
                raise serializers.ValidationError({
                    'address': 'This field is required for merchant creation.'
                })

            merchant_only_excluded_fields = [
                'id_card_image_1',
                'id_card_image_2',
                'license_image',
                'has_vehicle',
            ...
            for field in merchant_only_excluded_fields:
                attrs.pop(field, None)
        ...
        if 'vehicle_id' in self.initial_data:
            raise serializers.ValidationError({
                'vehicle_id': (
                    'Admin driver creation must use vehicle_type '
                    'and vehicle_number, not vehicle_id.'
                )
            })
        ...
        # address is not used for driver
...
class AdminRouteSerializer(serializers.ModelSerializer):
    ...
    zone = serializers.PrimaryKeyRelatedField(
        ...
        allow_null=True,
        help_text=(
            'Zone this route belongs to. Required for the route to be usable '
            'with the zone-subscription boarding system.'
        ),
    )
