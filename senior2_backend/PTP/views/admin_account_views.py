...
class AdminAccountsView(APIView):
        ...
        merchants = [
            {
                ...
                'address': (
                    merchant.merchant_profile.address
                    if hasattr(merchant, 'merchant_profile')
                    else ''
                ),
                'created_at': merchant.created_at,
            }
            for merchant in User.objects.filter(
                role=User.ROLE_MERCHANT
            )
            .select_related('merchant_profile')
            .order_by('-created_at')
        ]
    ...
    def post(self, request):
            ...
            return Response(
                {
                    ...
                    'account_status': account.account_status,
                    'address': (
                        account.merchant_profile.address
                        if hasattr(account, 'merchant_profile')
                        else ''
                    ),
                    'token': token.key,
        ...
        vehicle = Vehicle.objects.create(
            ...
            ownership=(
                'driver'
                if serializer.validated_data['has_vehicle']
                else 'government'
            ),
            route=route,
        )
