...
=====================
Manual vehicle status (is_full) endpoint has been removed.
Passenger count and is_full are now managed automatically
by the AI camera service via PassengerCountUpdateView.
"""
...
from PTP.serializers import DriverLocationSerializer
...
class DriverTripStatusView(DriverTrackingBaseView):
    def get(self, request):
        ...
        ).select_related('vehicle', 'route').first()
        ...
        return Response(
            {
                ...
                # Include live count in status response
                'passenger_count': vehicle.passenger_count if vehicle else 0,
                'capacity': vehicle.capacity if vehicle else 0,
                'occupancy_percentage': vehicle.occupancy_percentage if vehicle else 0.0,
                'is_full': vehicle.is_full if vehicle else False,
            },
...
class DriverTripStartView(DriverTrackingBaseView):
    def post(self, request):
        ...
        if driver.vehicle.route is None:
            return Response(
                {'detail': 'Driver vehicle does not have an assigned route.'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # Reset passenger count when a new trip starts
        driver.vehicle.passenger_count = 0
        driver.vehicle.is_full = False
        driver.vehicle.is_active = True
        driver.vehicle.save(update_fields=['passenger_count', 'is_full', 'is_active'])
...
class DriverTripStopView(DriverTrackingBaseView):
    def post(self, request):
        ...
        if trip is None:
            return Response(
                {'detail': 'There is no active vehicle tracking session.'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # Reset passenger count when trip ends
        if driver.vehicle:
            driver.vehicle.passenger_count = 0
            driver.vehicle.is_full = False
            driver.vehicle.save(update_fields=['passenger_count', 'is_full'])
...
class DriverLocationUpdateView(DriverTrackingBaseView):
    def post(self, request):
        ...
        if trip is None:
            return Response(
                {'detail': 'Start vehicle tracking before sending GPS locations.'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if route_check['is_off_route'] and not trip.route_alert_sent:

            NotificationService.send_driver_notification(
                driver_id=driver.driver_id,
                title="Route Deviation Warning",
                body=route_check['alert'],
            )
            trip.route_alert_sent = True
        trip.save(update_fields=['route_alert_sent'])
...
# NOTE: DriverVehicleStatusView has been intentionally removed.
# Passenger count and is_full are now managed automatically
# by the AI camera service via:
